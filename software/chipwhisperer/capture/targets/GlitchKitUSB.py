...
class GlitchKitUSB(TargetTemplate):
    """
    USB-based attacks for GlitchKit
    """
    ...
    def _con(self, scope=None):
        ...
        self.glitchkit  = self.greatfet.glitchkit
        self.usb        = self.greatfet.glitchkit.usb
    ...
    def readOutput(self):
        ...
        # Add the last captured descriptor to the glitch monitor.
        self.newInputData.emit(bytearray(self.last_descriptor))
    ...
    def go(self):
        ...
        self.greatfet.switch_to_external_clock()

        # Provide a phase-locked clock to the target from the GreatFET,
        # which will be enabled at the same time as VBUS.
        self.glitchkit.provide_target_clock(self.usb.VBUS_ENABLED);

        # Trigger when a setup packet is issued.
        self.glitchkit.trigger_on_events(self.usb.HOST_SETUP_TRANSFER_QUEUED)

        # Capture a descriptor, and store it for use in the readOutput stage.
        self.last_descriptor = self.usb.capture_control_in(
            request= self.usb.GET_DESCRIPTOR,
            value  = self.usb.GET_DEVICE_DESCRIPTOR,
            length = 18,
            ui_event_call = QCoreApplication.processEvents
        )
